...
export class ExportService {
  ...
  private async writePdf(
    ...
    const tempHtmlPath = path.join(
      ...
      `.marko-export-${Date.now()}-${exportTaskCounter}.html`,
    )
...
function validateExportOutputPath(value: unknown): string {
...
function createExportTaskId(format: ExportFormat): string {
  exportTaskCounter += 1
